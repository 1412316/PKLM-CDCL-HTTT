...
namespace PhanLop
{
    class ClassInfo
    {
        ...
        public static List<ClassInfo> ThongTinClass(String filetest, String fileknn)
        {
            List<Document> listTrue = Document.layCacDocumentsDaPhanClass(filetest);
            List<Document> listPhanLop = Document.layCacDocumentsDaPhanClass(fileknn);
            List<ClassInfo> listClass = new List<ClassInfo>();

            //Lay cac class trong file txt dung 
            for (int i = 0; i < listTrue.Count; i++)
            {
                int result = IsExist(listClass, listTrue[i].className);
                if (result == 0)
                {
                    listClass.Add(new ClassInfo(listTrue[i].className, 0, 0, 0, 0, 0, 0));
                }
            }
