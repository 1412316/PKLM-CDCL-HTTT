...
namespace PhanLop
{
    class Document
    {
        ...
        //====================Thêm vào====================
        public static List<Document> layCacDocumentsDaPhanClass(String filePath)
        {
            List<Document> documentList = new List<Document>();
            ...
            using (StreamReader sr = new StreamReader(filePath))
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    for (int i = 0; i < features.Length; i++)
                    {
                        ...
                        {
                            documentList.ElementAt(j).className = features[i];
                        }
                        else
                        {
                            //Tìm index của feature (tìm xem có feature đó trong document chưa)
        ...
        public static int demSoDocument(String filePath)
        {
            List<Document> listDoc = Document.layCacDocumentsDaPhanClass(filePath);
            return listDoc.Count;
        }
